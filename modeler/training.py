...
from configs.constants import (CALLBACK_DELAY_STEPS, CALLBACK_PATIENCE, EVALUATION_FREQUENCY, MODEL_ENV_ID,
                               MODEL_STRUCTURE_POLICY, OUTPUT_LOCATION, TOTAL_STEPS, TRAINING_BOUNDARY_LIMIT,
                               TRAINING_WINDOW_SIZE, VERBOSE)
...
class Trainer:
    def __init__(self, data: DataFrame, frame_bound_max: int = TRAINING_BOUNDARY_LIMIT,
        ...
        self.env = DummyVecEnv([lambda: gym.make(self.env_id, df=data, frame_bound=self.frame_bound,
                                                 window_size=window_size)])
        self.model = RecurrentPPO(policy, self.env, verbose=self.verbose)
    ...
    def save_mmodel(self):
        ...
        with open(self.output_location.joinpath("configs.json"), "w") as file:
            dump(configs, file, indent=4)

        self.model.save(self.output_location.joinpath("model.zip").as_posix())

        log.info(f"Stored the model at: {self.output_location.as_posix()}")


if __name__ == "__main__":
    fetched_data = get_stock_data(symbol="GOOG", local_data="../temp/GOOGL.csv")
